...
func (session *LoginSession) Connect() (LoginResponse, error) {
	...
	req, _ := http.NewRequest("POST", "http://172.16.0.46/eportal/InterFace.do", data)
	// 设置传输的数据
	query := req.URL.Query()
...
func (session *LoginSession) GetInfo() (UserInfoResponse, error) {
	...
	req, _ := http.NewRequest("POST", "http://172.16.0.46/eportal/InterFace.do", nil)
	// 设置传输的数据
	query := req.URL.Query()
